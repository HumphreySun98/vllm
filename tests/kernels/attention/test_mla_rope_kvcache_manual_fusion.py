...
"""Tests for the manual MLA RoPE + KV-cache-write fusion (RFC #43224,
issue #43504).

Three layers:

1. Seam-shape numerics: the fused kernel invoked with the *exact strided
   layouts* ``MultiHeadLatentAttentionWrapper.forward`` produces (``q_pe`` as
   a trailing slice of the full q tensor, ``k_pe`` as a split view of the
   kv_lora projection) must match the unfused custom-op pipeline
   (``ops.rotary_embedding`` + ``ops.concat_and_cache_mla``) that the
   non-fused path executes today.
2. Probe: ``mla_rope_kvcache_fusion_enabled`` enables only the configurations
   the fused kernel supports.
3. Wiring: ``MLAAttention.forward`` skips its own KV-cache update when the
   producer's ``kv_cache_dummy_dep`` is supplied.
"""
...
from vllm.model_executor.layers.mla import mla_rope_kvcache_fusion_enabled
...
def test_probe_happy_path_and_rejections(default_vllm_config):
    ...
    def probe(**overrides):
        ...
        return mla_rope_kvcache_fusion_enabled(**kwargs)
