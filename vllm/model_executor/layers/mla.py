...
def mla_rope_kvcache_fusion_enabled(
    ...
    (RFC #43224, ``fused_rope_unified_mla_kv_cache_update``) can be used.

    Mirrors the conditions under which ``MLARoPEKVCacheCatFusionPass``
    would have matched: the exact rope types its matchers cover, full-width
    rotation, kernel dtype parity, and the unfused-only cache layouts
    excluded.
    """
...
class MultiHeadLatentAttentionWrapper(PluggableLayer):
    ...
    def __init__(
        ...
        self._use_fused_rope_kv_cache = mla_rope_kvcache_fusion_enabled(
    ...
    def forward(
        ...
        if self._use_fused_rope_kv_cache:
            # Manual RoPE + KV-cache-write fusion (RFC #43224): rotates
