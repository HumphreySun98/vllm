...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def forward(
        ...
        """``kv_cache_dummy_dep``: when an upstream fused producer (manual
        RoPE + KV-cache fusion, RFC #43224) has already written this layer's
        KV cache, it passes the producer op's dummy output here. The KV-cache
        update below is then skipped, and the dummy is threaded into the
        attention op so torch.compile preserves the write -> attend order.
        """
...
def fused_rope_unified_mla_kv_cache_update_impl(
    ...
    """Fused RoPE (q_pe/k_pe, in place) + MLA KV-cache concat-and-write.

    Returns a dummy tensor that downstream attention takes as a data
    dependency so torch.compile preserves the write -> attend ordering.
    Used both by the manual fusion call site in
    ``MultiHeadLatentAttentionWrapper`` and as the replacement target of
    the ``MLARoPEKVCacheCatFusionPass`` compiler pass (RFC #43224).
    """
    layer_name = _resolve_layer_name(layer_name)
